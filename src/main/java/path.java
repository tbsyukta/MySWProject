...
public class path {
	...
	public void follopath() throws InterruptedException {
		...
		for (WebElement typee : typelist) {
			...
			if (sstText.equalsIgnoreCase(desired)) {
				typee.click();
				b = false;
				break;
		...
		if (b) {
			...
			driver.findElement(By.xpath("//input[contains(@id,'DXEFL_DXEditor13_I')]")).sendKeys("ShiftNumber05");

			// send key to shift description
			driver.findElement(By.xpath("//input[contains(@id,'DXEFL_DXEditor3_I')]")).sendKeys("ShiftNumber05Desc");

			// send key to sort number
			driver.findElement(By.xpath("//input[contains(@id,'DXEFL_DXEditor5_I')]")).sendKeys("050505");
			...
			for (WebElement cti : clcokintime) {
				...
				if (ctiText.equalsIgnoreCase("01:00")) {
					cti.click();
					break labeltest;
			...
			for (WebElement cto : clcokouttime) {
				...
				// System.out.println(eText);
				if (ctoText.equalsIgnoreCase("03:15")) {
					cto.click();
					break;
			...
			List<WebElement> self = driver.findElements(By.xpath("//table[contains(@id,'DXEditor14_DDD_L_LBT')]//tr"));
