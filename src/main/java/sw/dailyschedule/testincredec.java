...
public class testincredec {
	...
	public void follopath() throws InterruptedException {
		...
		for (int i = 0; i < list1.size(); i++) {
			...
			if (req == 0 && hrd == 0) {
				for (int m = 1; m <= 2; m++) {
					WebElement arrowUp = driver.findElement(
							By.id("ctl00_MainContent_ASPxCallbackPanelNotification_MainShiftGrid_DXCBtn" + n + "Img"));
					arrowUp.click();
					Thread.sleep(3000);
			...
			for (int k = 0; k < req; k++) {
				if (hrd < req) {
					...
					try {
						...
						WebElement confirmyes2 = driver.findElement(
								By.xpath("//div[@id='MainContent_ASPxCallbackPanelNotification_btnAddOK']"));
						confirmyes2.click();
					}
					Thread.sleep(2000);
					driver.switchTo().defaultContent();
